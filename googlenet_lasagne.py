...
x = T.tensor4('x')
out, params, class_names= apply_google_net(x)
f = theano.function([x], out)
img = np.array(Image.open('tobacconist-wall.jpg'))
img = np.swapaxes(np.swapaxes(img, 1, 2), 0, 1) / 255.0
img = img[np.newaxis].astype('float32')
print class_names[np.argmax(f(img))]
import ipdb; ipdb.set_trace() 
arr2=misc.imread('/data/lisatmp4/negar/datasets/centraldownsampled/s07-d72-cam-002/image-2.jpeg')
import ipdb; ipdb.set_trace()
